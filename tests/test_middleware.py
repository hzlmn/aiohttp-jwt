...
async def test_get_payload(create_app, aiohttp_client, fake_payload, token):
    ...
    response = await client.get(
        "/foo", headers={"Authorization": "Bearer {}".format(token.decode("utf-8"))}
    )
...
async def test_throw_on_wrong_token_scheme(
    ...
    client = await aiohttp_client(create_app(routes))
    response = await client.get(
        "/foo", headers={"Authorization": "Wrong {}".format(token.decode("utf-8"))}
    )
...
async def test_throw_on_wrong_header_format(
    ...
    routes = (("/foo", handler),)
    client = await aiohttp_client(create_app(routes))
    response = await client.get("/foo", headers={"Authorization": "Failed"})
    assert response.status == 403
...
async def test_forbidden_on_wrong_secret(create_app, aiohttp_client, fake_payload):
    ...
    response = await client.get(
        "/foo", headers={"Authorization": "Bearer {}".format(token)}
    )
    assert response.status == 403
...
def form_auth(scheme, correct):
    ...
    scheme = scheme + " {}"
    if correct:
        auth = scheme.format(jwt.encode({"foo": "bar"}, "secret").decode("utf-8"))
    else:
...
async def test_whitelisted_path(create_app, aiohttp_client, fake_payload):
    ...
    routes = (("/foo", handler),)
    client = await aiohttp_client(create_app(routes, whitelist=[r"/foo*"]))
    response = await client.get("/foo")
    assert response.status == 200
...
async def test_request_property(create_app, aiohttp_client, fake_payload, token):
    ...
    routes = (("/foo", handler),)
    client = await aiohttp_client(create_app(routes, request_property=request_property))
    response = await client.get(
        "/foo", headers={"Authorization": "Bearer {}".format(token.decode("utf-8"))}
    )
    assert response.status == 200
...
async def test_storing_token(create_app, aiohttp_client, fake_payload, token):
    ...
    routes = (("/foo", handler),)
    client = await aiohttp_client(create_app(routes, store_token=token_property))
    response = await client.get(
        "/foo", headers={"Authorization": "Bearer {}".format(token.decode("utf-8"))}
    )
    assert response.status == 200
...
async def test_token_getter(getter, create_app, aiohttp_client, fake_payload, token):
    ...
    routes = (("/foo", handler),)
    client = await aiohttp_client(create_app(routes, token_getter=getter))
    response = await client.get("/foo", params={"auth_token": token.decode("utf-8")})
    assert response.status == 200
...
@pytest.mark.parametrize("check", [is_revoked, is_revoked_coro])
